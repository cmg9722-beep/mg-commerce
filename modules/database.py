...
def seed_initial_data():
    ...
    # 공급사 8곳 (4/25 현재 최신 현황)
    suppliers = [
        (1, "우지 테크놀로지",     "五祉科技有限公司",          "五祉科技有限公司",          "광저우",   "A+A",    "-",
         "¥1.50", "¥4.5×200개=¥940", "-", "1",  "闪电棱镜 시리즈, 드롭쉬핑 지원 | 본발주완료(4/25) | 발송예정 4/28",
         "본발주완료", "샘플검수완료(4/23) | 본발주 ¥940 결제완료 | 발송예정 4/28(월)", "509835923285"),
        (2, "xmyydz1688",          "xmyydz1688",               "xmyydz1688",               "샤먼(厦门)", "공장입사", "-",
         "🆓 무료!", "¥4.00/롤 → 본발주 ¥4.0×200롤=¥800", "-", "1",  "무료 샘플 + 무료 배송! MG COMMERCE 라벨 인쇄 | 본발주완료(4/25)",
         "본발주완료", "샘플검수완료(4/23) | 본발주 ¥800 결제완료 | 운송장 월요일 수령예정", "509837025424"),
        (3, "오미스 스포츠 용품",  "奥美斯体育用品股份有限公司","오미스 스포츠 용품 주식회사","양저우",   "공장입사", "-",
         "¥5.20",  "¥4.80",     "¥4.50","2",  "OEM/ODM 가능 | 샘플검수완료(4/23) | 3색 공급조건 확인 필요 | 본발주 검수 후 결정",
         "샘플검수완료", "샘플수령완료(4/23) | 쿠팡이미지완료(베이지) | 입고불가 3건 확인필요 | 본발주협의중", "SF1566001528275(SF)"),
        (4, "원 세일 토이 팩토리", "暖帆玩具厂",                "원 세일 토이 팩토리",       "이우(义乌)", "A+A",   "80%",
         "¥4.20",  "¥4.20",     "¥4.20","1",  "USB 선풍기 ⑤ | 4/25 현재 무응답 4일 | 추가 답변없으면 라인업 제외 검토",
         "무응답", "4/25 기준 무응답 4일 경과 | 발주 대기 중단 | 내일도 무응답시 제외 확정", "509837025413"),
        # 추가 공급사 (Cowork Opus 발굴, 4/19)
        (2, "광동미고과기",        "广东米高科技有限公司",       "广东米高科技",              "광동",     "5년+",   "45%",
         "무료",   "¥1.80",     "¥1.50","10", "RoHS인증완료, 한국수출경험, 대응매우빠름 | 쿠팡이미지완료 | 카프톤 샘플 대안",
         "입고완료", "타배입고완료 | 쿠팡이미지완료 | 참고 샘플용", "SF1566001528275(SF)"),
        (4, "심천협성",            "深圳协成科技",               "-",                         "심천",     "20년+",  "-",
         "미정",   "미정",      "미정", "미정","BSCI/ISO9001/TUV인증, 한국수출경험 | USB 선풍기 대안 후보",
         "이메일회신대기", None, None),
        (4, "앤커럴(Ankeral)",     "Ankeral Technology Co., Ltd","leo@ankeral.com",           "심천",     "CE/FCC/RoHS","-",
         "미정","미정","미정","미정","USB 미니선풍기 S5×100+S17×100=200개 | 4/25 이메일 발송 | CE/FCC/RoHS인증, 월마트납품, OEM가능",
         "이메일답변대기", None, None),
        (5, "블루버터플라이(란디에)","佛山南海领途袜业有限公司",  "LINGTU",                    "불산",     "-",      "-",
         "미정",   "미정",      "미정", "미정","컬러 스포츠 손목밴드 ⑤ | 1688:lingtuwaye.1688.com | 4/13 이후 답장없음 → 재연락 필요",
         "무응답", None, None),
    ]
    for s in suppliers:
        conn.execute(
    ...
    timeline = [
        ...
        (1,"4/17","타배 창고 도착",1),(1,"4/19","한국 배송중",1),
        (1,"4/22","샘플 도착 수령",1),(1,"4/23","실물 촬영 완료",1),
        (1,"4/23","샘플 검수 완료",1),(1,"4/25","본발주 ¥940 결제완료 (200개)",1),
        (1,"4/28","발송 예정 (우지테크)",0),(1,"-","운송장 확인 후 타배 등록",0),
        # 공급사2 (카프톤 테이프) — 본발주완료
        ...
        (2,"4/17","타배 창고 도착",1),(2,"4/19","한국 배송중",1),
        (2,"4/22","샘플 도착 수령",1),(2,"4/23","실물 촬영 완료",1),
        (2,"4/23","샘플 검수 완료",1),(2,"4/25","본발주 ¥800 결제완료 (200롤)",1),
        (2,"-","정확한 운송장 번호 수령 예정(월요일)",0),
        # 공급사3 (손목보호대) — 샘플검수완료·본발주협의중
        (3,"4/9","1차 문의 발송",1),(3,"4/9","견적 회수 (100개 ¥4.80)",1),
        (3,"4/11","창고주소 전달",1),(3,"4/11","샘플 주문 메시지 발송",1),
        (3,"4/13","샘플 발송 확인",1),(3,"4/17","트래킹번호 입력 (SF1566001528275)",1),
        (3,"4/17","타배 창고 도착",1),(3,"4/20","착불 ¥23 해결완료",1),
        (3,"4/22","샘플 도착 수령",1),(3,"4/23","실물 촬영 완료",1),
        (3,"4/23","샘플 검수 완료",1),(3,"-","3색 공급조건 확인 후 본발주",0),
        (3,"-","쿠팡 입고불가 3건 매니저 확인",0),
        # 공급사4 (USB 선풍기/원세일토이) — 무응답 4일
        ...
        (4,"4/21","무응답 시작",1),(4,"4/25","무응답 4일째 — 내일도 무응답시 제외",0),
    ]
    for t in timeline:
    ...
    milestones = [
        ...
        ("배송비 결제 완료 (₩8,250×3)",             "done",     "4/19"),
        ("쿠팡 이미지 가공 완료 (3종)",             "done",     "4/19"),
        ("추가 공급사 발굴 (심천협성/앤커럴 등)",    "done",     "4/19"),
        ("샘플 한국 도착 수령",                     "done",     "4/22"),
        ("3종 실물 촬영 완료",                      "done",     "4/23"),
        ("3종 샘플 품질 검수 완료",                 "done",     "4/23"),
        ("쿠팡 Wing 상품 등록 (①②③)",             "done",     "4/23"),
        ("① ESD 팔찌 본발주 완료 (200개·¥940)",    "done",     "4/25"),
        ("② 카프톤 테이프 본발주 완료 (200롤·¥800)","done",     "4/25"),
        ("③ 손목보호대 공급조건 확인 + 본발주",    "progress", "4/26~27"),
        ("앤커럴 USB 선풍기 1688 링크 확인 + 주문", "progress", "4/28"),
        ("쿠팡 로켓그로스 입고 요청",               "todo",     "4/28~29"),
