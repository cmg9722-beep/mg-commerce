...
### 게임(game/rush.html)은 검증 스위트로

```bash
node game/test/run.mjs          # 2·3번을 대신한다. 전부 ✅ 여야 push
```

자세한 건 `game/test/README.md`. 새 기능에는 검사를 같이 낸다.
검사에서 ❌가 뜨면 **게임을 고치기 전에 검사를 먼저 읽는다** —
검사가 틀려서 멀쩡한 기능이 고장으로 잡힌 적이 있다.
